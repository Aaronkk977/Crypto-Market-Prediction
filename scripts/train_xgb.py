#!/usr/bin/env python
"""
Train XGBoost model with Walk-Forward CV and multi-seed ensemble.

Usage:
    python scripts/train_xgb.py
    python scripts/train_xgb.py --config configs/xgb.yaml
"""
...
from myproj.models import train_xgboost
...
def main():
    parser = argparse.ArgumentParser(description='Train XGBoost (multi-seed, walk-forward CV)')
    parser.add_argument('--config', type=str, default='configs/xgb.yaml')
    ...
    for fold_idx, X_train, X_val, y_train, y_val in fold_gen:
        ...
        # Average val pearson across seeds for this fold
        ...
        np.save(preds_dir / f'fold{fold_idx}_xgb_pred.npy', avg_val_preds)
    ...
    # --- Summary ---
    print("\n" + "=" * 60)
    print("WALK-FORWARD CV SUMMARY (XGBoost)")
    print("=" * 60)
